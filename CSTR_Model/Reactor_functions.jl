...
function linearise(linpoint::Array{Float64, 1}, h::Float64, model::Reactor)
  ...
  J21 = jac[2,1]
  J22 = jac[2,2]
  rfunc = reactor_ode(linpoint, 0.0, model) # u = 0 because Bs account for that term
  f0 = rfunc[1]
  g0 = rfunc[2]
  D = jac*linpoint
  D11 = D[1]
  ...
  A11 = 1.0 + ((J11^4*h^4)/24 + (J11^3*h^3)/6 + (J11^2*J12*J21*h^4)/8 + (J11^2*h^2)/2 + (J11*J12*J21*J22*h^4)/12 + (J11*J12*J21*h^3)/3 + J11*h + (J12^2*J21^2*h^4)/24 + (J12*J21*J22^2*h^4)/24 + (J12*J21*J22*h^3)/6 + (J12*J21*h^2)/2)*linpoint[1]
  ...
  A22 = 1.0 + ((J11^2*J12*J21*h^4)/24 + (J11*J12*J21*J22*h^4)/12 + (J11*J12*J21*h^3)/6 + (J12^2*J21^2*h^4)/24 + (J12*J21*J22^2*h^4)/8 + (J12*J21*J22*h^3)/3 + (J12*J21*h^2)/2 + (J22^4*h^4)/24 + (J22^3*h^3)/6 + (J22^2*h^2)/2 + J22*h)*linpoint[2]
