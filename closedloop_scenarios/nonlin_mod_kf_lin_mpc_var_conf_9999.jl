...
us[1] = MPC.mpc_var(kfmeans[:, 1], kfcovars[:,:, 1], horizon, A, B, b, aline, bline, cline, QQ, RR, ysp, usp[1], 20000.0, 1000.0, false, 1.0, Q, 18.4207, true) # get the controller input
tic()
for t=2:N
  ...
  if t%10==0
    us[t] = MPC.mpc_var(kfmeans[:, t], kfcovars[:,:, t], horizon, A, B, b, aline, bline, cline, QQ, RR, ysp, usp[1], 20000.0, 1000.0, false, 1.0, Q, 18.4207, true) # get the controller input
  else
    us[t] = us[t-1]
  end
end
